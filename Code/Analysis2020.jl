...
# Endurance ~ Mito
PhyNetLM_CIS3endur  = phyloNetworklm(@formula(CI_State3  ~ LogEndurance), dat_means, spe_net_rooted, model="lambda")
PhyNetLM_CIS4endur  = phyloNetworklm(@formula(CI_State4  ~ LogEndurance), dat_means, spe_net_rooted, model="lambda")
PhyNetLM_CIRCRendur = phyloNetworklm(@formula(CI_RCR     ~ LogEndurance), dat_means, spe_net_rooted, model="lambda")
PhyNetLM_CIIS3endur = phyloNetworklm(@formula(CII_State3 ~ LogEndurance), dat_means, spe_net_rooted, model="lambda")
PhyNetLM_CIIS4endur = phyloNetworklm(@formula(CII_State4 ~ LogEndurance), dat_means, spe_net_rooted, model="lambda")
PhyNetLM_CIIRCR     = phyloNetworklm(@formula(CII_RCR    ~ LogEndurance), dat_means, spe_net_rooted, model="lambda")
# get r^2
r2(<insertModelHere>)
